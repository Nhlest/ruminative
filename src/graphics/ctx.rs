...
impl RenderingContext {
  ...
  pub fn new() -> Result<Self, Box<dyn Error>> {
    ...
    let pipeline = Self::render_pass_and_pipeline(device.clone(), swapchain.clone(), vs.clone(), fs.clone())?;
    let (descriptor_set, previous_frame_end) =
      Self::sampler(device.clone(), queue.clone(), pipeline.clone(), &memory_allocator)?;
